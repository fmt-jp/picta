...
## v1.0後の変更: シャッターを持ち上げて大きくした

シャッターが画面の下端に近すぎて、片手だと指を伸ばす必要があった。

- 大きさを 96px → **115px**（1.2倍）
- 位置をボタン1個分（96px）持ち上げ

数値は `.camera-screen` の `--shutter-size` / `--shutter-lift` の2変数にまとめ、
ボタン・マーク・下の余白・ビューファインダーの上限サイズがすべてそこから
計算されるようにした。以前は 96px と 260px がCSSとTSXに散らばっていて、
片方だけ変えると正方形が崩れる状態だった。

縦に短い端末（`max-height: 720px`）では持ち上げ量を 40px に抑えている。
96px のままだとビューファインダーが 375px → 292px まで縮んでしまうため
（40px なら 348px）。もともと下端に近い画面では、持ち上げの必要も小さい。

実ブラウザ 390×780 / 375×667 で撮影画面を描画し、シャッターの座標と
ビューファインダーが正方形のままであることを確認。単体173件・E2E45項目とも通過。
