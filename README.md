...
- 記録の詳細 / 編集 / 削除。写真は全画面で拡大表示（ピンチ／ダブルタップ）
- 一覧から複数選択してまとめて削除
- メモとタグの検索、タグからの絞り込み
...
設定済みでトークンが使えない場合は、ビルド前の `Check token` で止まり、
理由（無効 / 権限不足 / リポジトリが見えない）を明示してエラーになります。
...
削除は「削除する」のみで、単体削除・まとめて削除のどちらも
**「端末に保存した写真は削除されません」** という注意書きを添えています。

端末側の写真をアプリから削除する手段は、プラットフォーム上ほぼありません。

| | 端末の写真を削除 | 理由 |
| --- | --- | --- |
| Web / PWA | **できない** | ブラウザには、自身がダウンロードしたファイルを削除する手段が無い |
| iOS (ネイティブ) | **できない** | 写真Appの写真の削除には PhotoKit の `deleteAssets` が必要で、利用しているプラグインに該当APIが無い |
| Android (ネイティブ) | 技術的には可能 | 保存時のファイルパスを控えてあるため削除できるが、UIには出していない |
