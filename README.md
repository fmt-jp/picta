...
- 一覧から複数選択してまとめて削除
- メモとタグの検索、タグからの絞り込み
- CSVエクスポートと、写真付きZIPバックアップ／**ZIPからの復元（インポート）**
- PWA（オフライン起動可能）
...
エクスポートした写真付きZIPを読み込んで復元できます（エクスポート画面）。

- 読み込む前に**内容（形式・バージョン・件数）を確認**してから実行します
- **すでにある記録（同じ `id`）は上書きせず飛ばします**。
  同じZIPを二度読み込んでも増えず、手元の編集も壊しません
- 旧名の `picta-export`（version 1・緯度経度の列なし）も読めます。
  列は名前で引くので、列の増減や並び替えに影響されません
- 撮影日時は `manifest.json` の `timezoneOffsetMinutes` を使って復元します
  （CSVの日時にはタイムゾーンが無いため）
- 写真は**バイト列のまま**復元するので、EXIF（撮影日時・位置・キャプション）も保たれます
- ZIPは**ストリームで2回読み**ます。1回目で `manifest.json` と `records.csv`、
  2回目で写真を1枚ずつ書き込むので、数百枚のバックアップでも一度に抱え込みません
- 写真がZIPに無い行や、日時が読めない行は飛ばし、件数を結果に出します
...
E2Eは Chromium の擬似カメラ・擬似位置情報を使い、実際に操作して検証します。

- 起動直後のカメラ、1:1のビューファインダーと9分割グリッド、シャッター
- 復帰時のカメラ再起動（トラックを停止して `pageshow` を送る）
- 撮影 → メモ → タグ → 保存、位置情報とEXIFの書き込み
- 一覧（新しい順・日付グループ・サムネイル）、詳細、編集、写真の拡大表示
- 検索（メモ／タグ／0件）、タグ一覧と絞り込み
- CSV / ZIPエクスポート（BOM・日本語・写真との対応・EXIF・緯度経度）
- 設定（保存データの保護状態、端末保存OFF時の警告）
- 削除（単体・まとめて）
...
ZIPからのインポートは**対応済み**です）
ZIPからのインポート（復元）も未実装ですが、`manifest.json` にバージョンを持たせることで
将来対応できる形式にしてあります。

## 開発ログ

各Phaseおよびv1.0以降の変更について、実装・テスト・確認内容・既知の問題を
[docs/DEVELOPMENT_LOG.md](docs/DEVELOPMENT_LOG.md) に時系列で記録しています。
末尾の「現在の状態」に、公開先・テスト状況・**あえて現状維持としている項目**・
積み残しをまとめてあります。
